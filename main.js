...
function ($, _, locale, Handlebars, apiProject, apiData, sampleRequest, semver, WebFont, clipboardJS, copyToClipboard) {
  ...
  api.forEach(function (entry) {
    if (entry.type && entry.type.toLowerCase() === 'post' && entry.body && entry.body.length > 0) {
      const entryBody = [...entry.body];
      entry.body = { fields: { Body: entryBody } }
    }
  });
  ...
  if (apiProject.template.withSampleRequest == null)
    apiProject.template.withSampleRequest = true;
  ...
  function initDynamic() {
    ...
    $('.json-schema').click(function (e) {
      ...
      hljs.highlightElement(document.querySelector('#json-pre code'));
    });
  ...
  function changeVersionCompareTo(e) {
    ...
    if (compareVersion && articleVersions[group][name][0] === selectedVersion || version === selectedVersion) {
      ...
      if (fields._hasTypeInBodyFields !== true && entry.body && entry.body.fields)
        fields._hasTypeInBodyFields = _hasTypeInFields(entry.body.fields);
